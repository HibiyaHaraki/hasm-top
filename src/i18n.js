...
const TRANSLATIONS = {
  en: {
    ...
    hasmMainApp: 'MAIN APP', hasmSubApps: 'HASM ECOSYSTEM',
    ...
    navWhatIsHasm: 'What is HASM?', navExample: 'Example', navBenefit: 'What is benefit of using HASM?', navUsecase: 'What is usecase of HASM?', navPhilosophy: 'Philosophy',
    philosophyKicker: 'PHILOSOPHY',
    philosophyTitle: 'Why HASM exists: separating what happened from what it means.',
    philosophyLead: 'A resume or achievement list is a flat line: one date, one entry, one meaning forever. HASM starts from a different question — what if a life, a career, or a project is not a line, but two layers that only sometimes agree?',
    philosophyProblemTitle: 'The problem with linear life records',
    philosophyProblemDesc: 'Traditional formats — résumés, CVs, achievement lists — force human activity into a flat, chronological sequence. They record a single “final” story with quiet confidence, but they cannot show that an experience was later reinterpreted, that the same event meant different things to different people, or that meaning kept evolving long after the event itself ended. Once a line is written, it fixes the past shut.',
    philosophyDualLayerTitle: 'A dual-layer answer: Causal-Semantic Separation',
    philosophyDualLayerDesc: 'HASM keeps two layers side by side instead of merging them into one: an objective layer that never changes, and a subjective layer that is free to change. Nothing about the objective past is lost when interpretation moves on.',
    philosophyCausalTitle: 'Objective / Causal Layer',
    philosophyCausalDesc: 'Immutable facts, processes, and achievements — what actually happened, and what physically or temporally caused what. Modeled as a directed graph of cause and effect, this layer never gets rewritten.',
    philosophySemanticTitle: 'Subjective / Semantic Layer',
    philosophySemanticDesc: 'Interpretations, re-evaluations, and narrative meaning attached by a person (or by different people) over time. This layer can be revised freely — a new understanding never has to overwrite or delete the old causal record.',
    philosophyReinterpretTitle: 'Meaning can change without rewriting history',
    philosophyReinterpretDesc: 'A failed project in 2019 can become “the turning point that taught me X” in 2024 — and both statements stay true, connected to the same unchanged causal facts. HASM structures this directly: an EXPERIENCE can be re-linked, re-tagged, or re-narrated at any time, while the PROCESS/ACHIEVEMENT it points to stays exactly as it was.',
    philosophyGroundingTitle: 'Grounded across disciplines, not just software',
    philosophyGroundingDesc: 'The causal/semantic split is not an arbitrary schema choice — it echoes ideas long studied elsewhere: autobiographical memory and narrative identity in cognitive psychology, sensemaking and reflective practice in HCI, provenance and knowledge graphs in knowledge engineering, and career construction theory in social science. HASM tries to give those ideas a concrete, editable, local-first structure.',
    philosophyVisionTitle: 'A living structure, not a static résumé',
    philosophyVisionDesc: 'The goal is a model that keeps growing with a person or a team: the same underlying graph can become a competence map, a dynamic work history, material for reflective feedback, or a shared experience network — because it was never flattened into a single, final narrative in the first place.',
  ...
  ja: {
    ...
    hasmMainApp: 'メインアプリ', hasmSubApps: 'HASM エコシステム',
    hasmSubAppsDescription: 'HASM モデルエディタ本体と、文書編集・カラーパレット設計・ブランドマーク幾何学を担う専門サブモジュールを見てみましょう。',
    navWhatIsHasm: 'What is HASM?', navExample: 'Example', navBenefit: 'What is benefit of using HASM?', navUsecase: 'What is usecase of HASM?', navPhilosophy: '思想',
    philosophyKicker: 'PHILOSOPHY',
    philosophyTitle: 'HASMが目指すもの─「起きたこと」と「その意味」を切り離す。',
    philosophyLead: '履歴書や実績リストは一本の線です。ひとつの日付に、ひとつの行、永久にひとつの意味。HASMは別の問いから出発します──人生やキャリア、プロジェクトは一本の線ではなく、時にしか一致しない二つの層でできているのではないか、と。',
    philosophyProblemTitle: '線形な人生記録が抱える問題',
    philosophyProblemDesc: '履歴書やCV、実績リストのような従来のフォーマットは、人の活動をフラットで時系列な並びに押し込めてしまいます。ひとつの「最終的な物語」を静かな自信とともに記録しますが、ある経験が後から再解釈されたこと、同じ出来事が人によって異なる意味を持っていたこと、出来事が終わった後も意味づけが変わり続けたことは示せません。一度書かれた行は、過去を固く閉じてしまいます。',
    philosophyDualLayerTitle: '二層構造という答え──Causal-Semantic Separation（因果・意味分離）',
    philosophyDualLayerDesc: 'HASMはこの二つを一つに混ぜ込まず、並べて保持します。変わらない客観層と、自由に変わってよい主観層です。解釈が先に進んでも、客観的な過去は何ひとつ失われません。',
    philosophyCausalTitle: '客観層（Causal Layer）',
    philosophyCausalDesc: '実際に起きたこと、何が何を引き起こしたのかという不変の事実・プロセス・成果。有向グラフとしてモデル化されるこの層は書き換えられることがありません。',
    philosophySemanticTitle: '主観層（Semantic Layer）',
    philosophySemanticDesc: 'ある人（あるいは異なる複数の人）が時間をかけて与える解釈・再評価・語り。この層は自由に更新でき、新しい理解が古い客観的記録を上書き・削除する必要はありません。',
    philosophyReinterpretTitle: '歴史を書き換えずに、意味だけを更新できる',
    philosophyReinterpretDesc: '2019年の失敗したプロジェクトは、2024年には「Xを学んだ転機」になり得ます──そしてどちらの記述も真実のまま、変わらない同じ客観的事実に結びついています。HASMはこれを直接構造化します。EXPERIENCEはいつでも再リンク・再タグ付け・再ナラティブ化できる一方で、その先にあるPROCESS/ACHIEVEMENTはそのままの姿で残ります。',
    philosophyGroundingTitle: 'ソフトウェアだけでなく、複数の学問領域に根ざす',
    philosophyGroundingDesc: '因果と意味の分離は恩意的なスキーマ設計ではありません。認知心理学における自伝的記憶・物語的同一性、HCIにおけるセンスメイキングと内省的実践、知識工学における来歴（プロベナンス）とナレッジグラフ、社会科学におけるキャリア構築理論──これらの知見に具体的で編集可能、かつローカルファーストな構造を与えることを目指しています。',
    philosophyVisionTitle: '静的な履歴書ではなく、生き続ける構造',
    philosophyVisionDesc: '目指すのは、人やチームとともに成長し続けるモデルです。同じグラフが、コンピテンシーマップにも、動的な職務経歴にも、内省的フィードバックの材料にも、共有される経験ネットワークにもなり得ます。なぜならそれは、最初から単一の最終的な物語へと平坦化されていないからです。',
